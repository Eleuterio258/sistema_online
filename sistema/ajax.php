<?php 
    ...
    if(!empty($_POST)){
        ...
        if($_POST['action'] == 'searchCliente')
        {
            if(!empty($_POST['cliente'])){
                ...
                $query = mysqli_query($conection, "SELECT * FROM cliente WHERE idcliente LIKE '$id_cliente' and estatus = 1");
                mysqli_close($conection);
                $result = mysqli_num_rows($query);
    ...
    if($_POST['action'] == 'updateDataEmpresa'){
        ...
        empty($_POST['txtEmailEmpresa']) || empty($_POST['txtDirEmpresa']) || empty($_POST['txtIva']))
        {
            ...
            $queryUpd = mysqli_query($conection,"UPDATE configuracion SET rfc = $intRfc,
                                                                    nombre = '$strNombre',
